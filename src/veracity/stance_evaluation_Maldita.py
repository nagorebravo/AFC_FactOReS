...
def plot_confusion_matrix2(y_pred, y_true, labels, model_name, stance, output_path):
    ...
    src_labels = [x["evidence"][0]["stance"].lower() for x in y_pred]
    tgt_labels = [x["STANCE"].lower() for x in y_true]
    ...
    plt.title(f"{title_name} - {model_name} - {stance} - Confusion Matrix")
    plt.tight_layout()
...
def plot_confusion_matrix(y_pred, y_true, labels, model_name, stance, output_path):
    ...
    # Aumentar tamaño solo de los nombres de las clases
    ax.set_xticklabels(SHORT_LABELS, fontsize=20)
    ax.set_yticklabels(SHORT_LABELS, fontsize=20)

    # Cambiar tamaño de las etiquetas de ejes y ticks
    ...
    plt.title(f"{title_name} - {model_name} - {stance} - Confusion Matrix", fontsize=20)
...
def plot_class_metrics2(y_pred, y_true, labels, model_name, stance, output_path):
    ...
    plt.figure(figsize=(10, 6))
    for i, metric in enumerate(metrics):
        bars = plt.bar(x + i * width, data[metric], width=width, label=metric.capitalize())  # <-- Capitalize
        for bar in bars:
            height = bar.get_height()
            plt.text(bar.get_x() + bar.get_width()/2.0, height + 0.01, f'{height:.3f}', 
                     ha='center', va='bottom', fontsize=8)

    plt.xticks(x + width, [lbl.capitalize() for lbl in labels])  # <-- Capitalize
    plt.ylabel("Score")
...
def plot_class_metrics(y_pred, y_true, labels, model_name, stance, output_path):
    ...
    plt.ylabel("F1-Score")
    plt.xlabel("Stance Label")
    plt.title(f"{title_name} - {model_name} - {stance} - F1-score por clase")
    plt.ylim(0, 1.1)
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close()
    ...
    def evaluate_averitec_score(self, srcs, tgts):
        ...
        for src, tgt in zip(srcs, tgts):
            ...
            for i, level in enumerate(self.averitec_reporting_levels):
                if score > level:
                    this_example_scores[i] = src["stance"] == tgt["STANCE"]
            scores.append(this_example_scores)
        return np.mean(np.array(scores), axis=0)

    
    def evaluate_veracity(self, src, tgt):
        src_labels = [x["evidence"][0]["stance"].lower() for x in src]
        tgt_labels = [x["STANCE"].lower() for x in tgt]
...
def load_true_labels(path):
    with open(path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    return {item['claim'].lower(): item['STANCE'].lower() for item in data}
...
def evaluate_predictions_distribution(predictions_path, true_labels_path):
    ...
    for item in preds:
        ...
        pred_label = item["evidence"][0]['stance'].lower()
...
if __name__ == "__main__":
    ...
    for mode in modes:
        ...
        output_dir = "src/veracity/st_eval_results/" + output_dataset + "/" + mode
